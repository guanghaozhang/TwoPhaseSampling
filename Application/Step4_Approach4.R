...
Target.sub.dist$N_mgi <- round(Target.sub.dist$pr*20000,0)
...
my_application=function(dat, budget=500000, n=330000000, c0=10000, c1=0.01,npilot = 100){
  ...
  #pilot sample for approach 4
  setDT(mgi.sample)
  setkey(mgi.sample, agec, race,sex)
  mgi.sample.dist <- mgi.sample[, .N, keyby = list(agec, race,sex)]
  mgi.sample.dist.merge <- merge(mgi.sample.dist, Target.sub.dist, by=c("agec","race","sex"), all.x=T, all.y=F)
  mgi.sample.dist.merge$N_mgi <- apply(cbind(mgi.sample.dist.merge$N.x, mgi.sample.dist.merge$N_mgi), 1, min)
  mgi.sample.new <- data.table(strata(data=mgi.sample, stratanames=c("agec", "race","sex"), 
                                      size=mgi.sample.dist.merge$N_mgi, method="srswor"))
  mgi.sample$ID_unit <- seq(1,dim(mgi.sample)[1])
  mgi.sample.new <- merge(mgi.sample.new, mgi.sample, by="ID_unit", all.x=T, all.y=F)
  ne.new <- length(mgi.sample.new$lambda1)
  colnames(mgi.sample.new)[which(names(mgi.sample.new) == "agec.x")] <- "agec"
  colnames(mgi.sample.new)[which(names(mgi.sample.new) == "sex.x")] <- "sex"
  nrow(mgi.sample.new)
  ...
  Zp.new = model.matrix(~mgi.sample.new$bmic+mgi.sample.new$age+mgi.sample.new$racec+mgi.sample.new$sexc+mgi.sample.new$smkc)
  ...
  gamma = fitv$gamma
  v.v = exp(Zp.new%*%gamma) 
  v.v.2 = exp(Zp.2%*%gamma) 
  
  #Estimate Epp
  a <- 100
  b <- 5
  c2.v = a+b*mgi.sample.new$bmic+b*mgi.sample.new$age+b*mgi.sample.new$racec+b*mgi.sample.new$sexc+b*mgi.sample.new$smkc
  ...
  lambda1.v.new <- mgi.sample.new$lambda1
  Bprime.new <- budget - c0 - nrow(mgi.sample.new)*c1
  lambda2.opt.new = pmin(sqrt(v.v/c2.v)*Bprime.new/(lambda1.v.new*n*Epp), 0.9999)
  
  #study sample
  mgi.sample.new$r2 = rbinom(ne.new,1,lambda2.opt.new)
  sum(mgi.sample.new$r2)
  summary(lambda2.opt.new)
  study.new <- mgi.sample.new[mgi.sample.new$r2==1,]
  
  fite2 = glm(y~factor(bmic)+age+factor(racec)+factor(sex)+factor(smkc), data=study.new, family=binomial)
  
  mgi.sample.new$e2=predict(fite2, newdata=mgi.sample.new, type = 'response')
  
  fite3 = glm(e2 ~ age+factor(racec)+factor(sex), data=mgi.sample.new, family=binomial)
  
  mgi.sample.new$e3 = predict(fite3, newdata=mgi.sample.new, type = 'response')
  
  Target.sub$e3 = predict(fite3, newdata=Target.sub, type = 'response') 
  
  #Estimate beta
  parta = (mgi.sample.new$r2)*(mgi.sample.new$y)/(mgi.sample.new$lambda1*lambda2.opt.new)
  
  partb = (mgi.sample.new$r2-lambda2.opt.new)*(mgi.sample.new$e2)/(mgi.sample.new$lambda1*lambda2.opt.new)
  
  partc = (mgi.sample.new$e3)/(mgi.sample.new$lambda1)
  
  beta.opt.new = (sum(parta)-sum(partb)-sum(partc)+sum(Target.sub$e3)*(n/nrow(Target.sub)))/n
  beta <- c(beta.opt.new)
  return(beta)
}
...
##########b1
bootbeta <- matrix(ncol = 1, nrow = index_length)
...
CI = c(mean_beta[1]-qnorm(0.975)*sqrt(var_beta[1]), mean_beta[1]+qnorm(0.975)*sqrt(var_beta[1]))

save(bootbeta, mean_beta, var_beta, CI, file = "app_finalscript5_a4.RData")
